...
%%% table of contents
\renewcommand\contentsname{\Huge\sourcesansproextreme\normalfont\scshape\slshape Sommaire}
...
\definecolor{accent}{HTML}{$if(theme_color)$$theme_color$$else$607D8B$endif$}
\pagecolor{accent}
\color{base07}
\begin{center}
\hspace{-3cm}
\includegraphics[width=4cm]{../input/logo.pdf}
\vspace{2cm}
\end{center}
$if(title)$
\noindent
\sourcesansproextreme\mdseries\Huge{$title$}
\par
\vspace{-1cm}
$endif$
...
$if(author)$
\hspace{-3cm}
\sourcesansprolight\mdseries\large{$for(author)$$author$$sep$ \and $endfor$}
$endif$
\end{center}
\end{titlepage}
